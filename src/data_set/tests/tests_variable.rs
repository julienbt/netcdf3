use crate::{DataSet, DataType, DimensionType, InvalidDataSet};
...
fn test_add_var_error_invalid_name() {
    ...
    assert_eq!(
        ...
        data_set
            .add_var_i8::<&str>(INVALID_VAR_NAME, &vec![])
            .unwrap_err()
    );
...
fn test_add_var_error_already_exists() {
    ...
    assert_eq!(
        ...
        data_set
            .add_var_i32::<String>(VAR_NAME, &vec![])
            .unwrap_err()
    );
...
fn test_add_var_error_unlim_dim_first() {
    ...
    data_set
        ...
        .unwrap();
    data_set
        .set_unlimited_dim(UNLIMITED_DIM_NAME, UNLIMITED_DIM_SIZE)
        .unwrap();
    ...
    assert_eq!(
        ...
        data_set.dim_type(FIXED_DIM_NAME)
    );
    assert_eq!(true, data_set.has_dim(UNLIMITED_DIM_NAME));
    assert_eq!(
        Some(UNLIMITED_DIM_SIZE),
        data_set.dim_size(UNLIMITED_DIM_NAME)
    );
    assert_eq!(
        Some(DimensionType::UnlimitedSize),
        data_set.dim_type(UNLIMITED_DIM_NAME)
    );
    ...
    assert_eq!(
        InvalidDataSet::UnlimitedDimensionMustBeDefinedFirst {
            ...
            get_dim_names: INVALID_DIM_LIST
                .iter()
                .map(|s: &&str| String::from(*s))
                .collect()
        },
        data_set
            .add_var_i8(VAR_NAME, &INVALID_DIM_LIST[..])
            .unwrap_err()
    );
...
fn test_add_var_error_undef_dim() {
    ...
    const DIM_SIZE_1: usize = 10;
    const DIM_SIZE_2: usize = 20;
    const INVALID_DIM_LIST: [&str; 4] =
        [UNDEF_DIM_NAME_1, DIM_NAME_1, DIM_NAME_2, UNDEF_DIM_NAME_2];
...
fn test_rename_var() {
    ...
    data_set
        .add_var_i32::<&str>(VAR_NAME_1, &[DIM_NAME])
        .unwrap();
...
fn test_rename_var_error_already_exists() {
    ...
    data_set
        .add_var_i32::<&str>(VAR_NAME_1, &[DIM_NAME_1])
        .unwrap();
    data_set.add_fixed_dim(DIM_NAME_2, VAR_DATA_2_LEN).unwrap();
    data_set
        .add_var_i32::<&str>(VAR_NAME_2, &[DIM_NAME_2])
        .unwrap();
