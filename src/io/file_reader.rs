...
impl FileReader {
    ...
    pub fn read_record(&mut self, var_name: &str, record_index: usize) -> Result<DataVector, ReadError> {
        ...
        let record_offset: u64 =
            (i64::from(var_info.begin_offset.clone()) as u64) + ((record_index * self.data_set.record_size().unwrap_or(0)) as u64);
    ...
    fn parse_magic_word(input: &[u8]) -> Result<(&[u8], &[u8]), ParseHeaderError> {
        let (input, tag_value): (&[u8], &[u8]) =
            tag(&b"CDF"[..])(input).map_err(|err: NomError| ParseHeaderError::new(err, ParseHeaderErrorKind::MagicWord))?;
        Ok((input, tag_value))
    }
    ...
    fn parse_non_neg_i32(input: &[u8]) -> Result<(&[u8], i32), ParseHeaderError> {
        verify(be_i32, |number: &i32| *number >= 0_i32)(input)
    ...
    fn parse_name_string(input: &[u8]) -> Result<(&[u8], String), ParseHeaderError> {
        ...
        let (input, name): (&[u8], String) = map_res(take(num_of_bytes), |bytes: &[u8]| String::from_utf8(bytes.to_vec()))(input)
    ...
    fn parse_vars_list(input: &[u8], version: Version) -> Result<(&[u8], Vec<VariableParsedMetadata>), ParseHeaderError> {
        ...
        fn parse_offset(input: &[u8], version: Version) -> Result<(&[u8], Offset), ParseHeaderError> {
            match version {
                Version::Classic => be_i32(input).map(|(input, num_of_bytes): (&[u8], i32)| (input, Offset::I32(num_of_bytes))),
                Version::Offset64Bit => be_i64(input).map(|(input, num_of_bytes): (&[u8], i64)| (input, Offset::I64(num_of_bytes))),
            }
