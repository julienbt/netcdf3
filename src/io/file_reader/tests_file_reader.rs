...
use crate::{
    error::parse_header_error::{InvalidBytes, ParseHeaderError, ParseHeaderErrorKind},
    error::ReadError,
    io::compute_padding_size,
    Attribute, DataSet, DataType, Dimension, DimensionType, FileReader, Variable, Version,
};

use copy_to_tmp_file::{
    copy_bytes_to_tmp_file, EMPTY_DATA_SET_FILE_BYTES, EMPTY_DATA_SET_FILE_NAME, NC3_CLASSIC_FILE_BYTES, NC3_CLASSIC_FILE_NAME,
    NC3_ZERO_SIZED_UNLIMITED_DIM_FILE_BYTES, NC3_ZERO_SIZED_UNLIMITED_DIM_FILE_NAME, SCALAR_VARIABLES_FILE_BYTES,
...
fn test_read_var_i8() {
    ...
    assert_eq!(
        ReadError::VariableMismatchDataType {
            var_name: String::from(TEMP_U8_VAR_NAME),
            req: DataType::U8,
            get: DataType::I8
        },
    ...
    assert_eq!(
        ReadError::VariableMismatchDataType {
            var_name: String::from(TEMP_I16_VAR_NAME),
            req: DataType::I16,
            get: DataType::I8
        },
    ...
    assert_eq!(
        ReadError::VariableMismatchDataType {
            var_name: String::from(TEMP_I32_VAR_NAME),
            req: DataType::I32,
            get: DataType::I8
        },
    ...
    assert_eq!(
        ReadError::VariableMismatchDataType {
            var_name: String::from(TEMP_F32_VAR_NAME),
            req: DataType::F32,
            get: DataType::I8
        },
    ...
    assert_eq!(
        ReadError::VariableMismatchDataType {
            var_name: String::from(TEMP_F64_VAR_NAME),
            req: DataType::F64,
            get: DataType::I8
        },
...
fn test_parse_name_string() {
    {
        ...
        // Test a latin-1 word (not valid UTF-8)
        {
            ...
            assert_eq!(InvalidBytes::Bytes(vec![b'c', b'a', b'f', b'\xe9']), parsing_err.invalid_bytes,);
        }
    ...
    // Test a NetCDF-3 file conatining a zero-sized unlimited dimension but in which the number of records is indeterminated
    {
        ...
        let original_dataset: DataSet = {
            let (tmp_dir, input_file_path) =
                copy_bytes_to_tmp_file(NC3_ZERO_SIZED_UNLIMITED_DIM_FILE_BYTES, NC3_ZERO_SIZED_UNLIMITED_DIM_FILE_NAME);
            let file_reader: FileReader = FileReader::open(input_file_path).unwrap();
            let data_set: DataSet = file_reader.close().0;
            tmp_dir.close().unwrap();
            data_set
...
fn test_read_record_i8() {
    ...
    // Read the 1st record
    assert_eq!(
        Ok(TEMP_I8_VAR_DATA[0..15].to_vec()),
        file_reader.read_record_i8(TEMP_I8_VAR_NAME, 0)
    );
    // Read the 2nd records
    assert_eq!(
        Ok(TEMP_I8_VAR_DATA[15..30].to_vec()),
        file_reader.read_record_i8(TEMP_I8_VAR_NAME, 1)
    );
    assert_eq!(
...
fn test_read_record_u8() {
    ...
    // Read the 1st record
    assert_eq!(
        Ok(TEMP_U8_VAR_DATA[0..15].to_vec()),
        file_reader.read_record_u8(TEMP_U8_VAR_NAME, 0)
    );
    // Read the 2nd records
    assert_eq!(
        Ok(TEMP_U8_VAR_DATA[15..30].to_vec()),
        file_reader.read_record_u8(TEMP_U8_VAR_NAME, 1)
    );
    assert_eq!(
        ReadError::RecordIndexExceeded {
            index: 2,
...
fn test_read_record_i16() {
    ...
    // Read the 1st record
    assert_eq!(
        Ok(TEMP_I16_VAR_DATA[0..15].to_vec()),
        file_reader.read_record_i16(TEMP_I16_VAR_NAME, 0)
    );
    // Read the 2nd records
    assert_eq!(
        Ok(TEMP_I16_VAR_DATA[15..30].to_vec()),
        file_reader.read_record_i16(TEMP_I16_VAR_NAME, 1)
    );
    assert_eq!(
        ReadError::RecordIndexExceeded {
            index: 2,
...
fn test_read_record_i32() {
    ...
    // Read the 1st record
    assert_eq!(
        Ok(TEMP_I32_VAR_DATA[0..15].to_vec()),
        file_reader.read_record_i32(TEMP_I32_VAR_NAME, 0)
    );
    // Read the 2nd records
    assert_eq!(
        Ok(TEMP_I32_VAR_DATA[15..30].to_vec()),
        file_reader.read_record_i32(TEMP_I32_VAR_NAME, 1)
    );
    assert_eq!(
        ReadError::RecordIndexExceeded {
            index: 2,
...
fn test_read_record_f32() {
    ...
    // Read the 1st record
    assert_eq!(
        Ok(TEMP_F32_VAR_DATA[0..15].to_vec()),
        file_reader.read_record_f32(TEMP_F32_VAR_NAME, 0)
    );
    // Read the 2nd records
    assert_eq!(
        Ok(TEMP_F32_VAR_DATA[15..30].to_vec()),
        file_reader.read_record_f32(TEMP_F32_VAR_NAME, 1)
    );
    assert_eq!(
        ReadError::RecordIndexExceeded {
            index: 2,
...
fn test_read_record_f64() {
    ...
    // Read the 1st record
    assert_eq!(
        Ok(TEMP_F64_VAR_DATA[0..15].to_vec()),
        file_reader.read_record_f64(TEMP_F64_VAR_NAME, 0)
    );
    // Read the 2nd records
    assert_eq!(
        Ok(TEMP_F64_VAR_DATA[15..30].to_vec()),
        file_reader.read_record_f64(TEMP_F64_VAR_NAME, 1)
    );
    assert_eq!(
        ReadError::RecordIndexExceeded {
            index: 2,
